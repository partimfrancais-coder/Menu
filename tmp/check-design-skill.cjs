...
(async()=>{
 ...
 await page.route('http://menu.test/**',async route=>{
  ...
  if(p.endsWith('/design-skill')){
   ...
   return route.fulfill({json:{restaurantName:r.name,skillName:'koi-menu-pdf',designPrompt:'Saved prompt for '+r.name,documents:[{name:'SKILL.md',content:fs.readFileSync('ai-skills/koi-menu-pdf/SKILL.md','utf8')+'\n<script>window.injected=true</script>\n'+r.name},{name:'references/workflow.md',content:'Supporting workflow'}]}});
  }
 ...
 await page.getByRole('button',{name:'View design skill',exact:true}).click();await page.locator('#design-skill-document').waitFor();await page.screenshot({path:'tmp/design-skill-desktop.png'});
 await page.setViewportSize({width:390,height:844});await page.screenshot({path:'tmp/design-skill-mobile.png'});assert(await page.evaluate(()=>document.querySelector('#dialog').scrollWidth<=innerWidth));
