...
Use the original vector letter outlines for Small Bites and Asian; Arial Narrow was rejected. Leading icons sit slightly further left, with a visible gap before dish text. Do not draw black internal separator lines in Seafood or Beef. Preserve the current saved category order and omit empty specials groups. The approved preview is `assets/kuningan-proof.png`; content and prices in that image are historical, not live inputs. See the approved refinements in the workflow reference for implementation details.
