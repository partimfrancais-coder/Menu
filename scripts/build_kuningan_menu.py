...
DATA=json.loads((ROOT/'tmp/pdfs/kuningan-current.json').read_text(encoding='utf-8'))
SOURCE=ROOT/'Kuningan Lunch & Dinner 20260606A.pdf'
OUT=ROOT/'output/pdf/KOI-Kuningan-Lunch-Dinner-20260917.pdf'
...
FONTS={
 'regular':fitz.Font(fontfile='C:/Windows/Fonts/calibri.ttf'),
 'bold':fitz.Font(fontfile='C:/Windows/Fonts/calibrib.ttf'),
}
for key,name in [('regular','calibri.ttf'),('bold','calibrib.ttf')]:
 page.insert_font(fontname=key,fontfile='C:/Windows/Fonts/'+name)
RED=(.90,.045,.085);BLACK=(0,0,0)
categories={c['name']:c for c in DATA['categories']}
presets=json.loads((ROOT/'dist/menu-icons.js').read_text(encoding='utf-8').split('globalThis.MenuIconSet=',1)[1].rstrip(';\n'))
...
def heading(name,x,y,width):
 ...
 left=x+(width-bw)/2
 vector(box,(left,y,left+bw,y+bh))
...
def draw_item(item,x,y,width,size=12,shared_options=None):
 ...
 for j,tag in enumerate(leaders):icon(tag,x-19-17*(len(leaders)-1-j),y-4,22)
...
def draw_category(name,x,y,width):
 ...
 for n,i in enumerate(visible):
  ...
  if name=='Seafood' and i['name']=='Crispy Kalan':
   page.draw_line((x,y-2),(x+width,y-2),color=BLACK,width=.5);y+=2
  if name=='Beef' and i['name']=='Vietnamese Steak':
   page.draw_line((x,y-2),(x+width,y-2),color=BLACK,width=.5);y+=2
...
columns=[
 (78,319,['Soups','Appetizers','Salads','Sandwiches','Burger'],945),
 (424,330,['Pastas','Noodles & Rice','Poultry'],795),
 (778,312,['Seafood','Beef','Grill'],795),
 (1120,332,['Lamb','Poke Bowl','Sauce','Side Dishes','Kids'],925),
]
...
assert y<1028,y
text(77,1037,'KOI Kuningan | Lunch & Dinner | 17 September 2026',9)
center(W/2,1053,DATA['footer'],9)

expected=[i for c in DATA['categories'] for i in c['items'] if i['available']]
assert len(audit)==len(expected)==103,(len(audit),len(expected))
...
final.set_metadata({'title':'KOI Kuningan - Lunch & Dinner','author':'KOI','subject':'Menu proof from current saved menu data, 17 September 2026'})
final.subset_fonts()
final.save(OUT,garbage=4,deflate=True)
(ROOT/'tmp/pdfs/kuningan-audit.json').write_text(json.dumps({'items':audit,'count':len(audit),'pagePoints':[W/3,H/3],'unresolved':['Rico… a raviolis: source spelling remains unconfirmed.']},ensure_ascii=False,indent=2),encoding='utf-8')
